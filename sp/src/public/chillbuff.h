...
/** @private */
static _inline void _chillbuff_printerr(const char* error, const char* origin)
{
    ...
    if (error_msg != NULL)
    {
        _snprintf(error_msg, error_length, "\nCHILLBUFF ERROR: (%s) %s\n", origin, error);
 ...
 */
static _inline int chillbuff_set_error_callback(void (*error_callback)(const char*))
{
 ...
 */
static _inline void chillbuff_unset_error_callback()
{
 ...
 */
static _inline int chillbuff_init(chillbuff* buff, const size_t initial_capacity, const size_t element_size, const chillbuff_growth_method growth_method)
{
    if (buff == NULL)
    {
 ...
 */
static _inline void chillbuff_free(chillbuff* buff)
{
    if (buff == NULL)
    {
        return;
 ...
 */
static _inline void chillbuff_clear(chillbuff* buff)
{
    if (buff == NULL)
    {
        return;
